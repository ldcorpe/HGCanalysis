...
using namespace std;
// --------- MAIN -------------------
void PhotonIDMVA_Training_PH1(TString Level="", bool skipBadEvents=true)
{

if(Level!="PH1_A0_PU50" &&  Level!="PH1_A1k_PU140") {
std::cout << "ERROR you must specify option PH1_A0_PU50 or PH1_A1k_PU140"<<std::endl;
return;}
 ...
 // int nEvents= std::atoi(Nevent.Data());
  ...
  TFile *inputS1 = TFile::Open(path + "HoverE_"+Level+"_weight.root");
 // TFile *inputS1 = TFile::Open(path + "HoverEHgg_PH1age1kPU140.root ");
...
if(Level =="PhotonID") {
   ...
   /* weightB[0]=0;
	...
	if(Level =="PH1_A0_PU50"){	
		outputFileName = "PH1_A0_PU50_PhotonID";
	}
	if(Level =="PH1_A1k_PU140"){	
		outputFileName = "PH1_A1k_PU140_PhotonID";
	}
	...
	// -- variables
//	if(Level =="PhotonID") {
    factory->AddVariable("sigmaIetaIeta" );
...
//	factory->AddBackgroundTree( treeB3,  weightB[3] );
	...
	TCut mycutb ="";// " leadPho_PToM> (60./120.) && sublPho_PToM> (30./120.)";
		mycuts ="pt>30 && pt<100 && abs(etaSC)>1.6 && abs(etaSC)<2.5 && matchIndex>-1 ";// Might need a better discriminant LC
		mycutb ="pt>30 && pt<100 && abs(etaSC)>1.6 && abs(etaSC)<2.5";// Might need a better discriminant LC
	//	mycutb ="pt>30 &&  abs(etaSC)>1.6 && abs(etaSC)<2.5 && lengthCompatibility>-10";// 
