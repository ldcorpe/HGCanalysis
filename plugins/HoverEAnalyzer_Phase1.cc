// -*- C++ -*-
//
// Package:    HoverEAnalyzer_Phase1
// Class:      HoverEAnalyzer_Phase1
// 
//
//
...
// .h class info
class HoverEAnalyzer_Phase1 : public edm::EDAnalyzer {
	public:
		explicit HoverEAnalyzer_Phase1(const edm::ParameterSet&);
		~HoverEAnalyzer_Phase1();

		static void fillDescriptions(edm::ConfigurationDescriptions& descriptions);

	//	double fillEmIdVars(const edm::Ptr<reco::SuperCluster>& sc,const edm::PtrVector<reco::PFCluster>& clusters,const  edm::PtrVector<PFRecHit>& rcs, const HGCalGeometry *geom , const edm::Event& iEvent, const edm::EventSetup& iSetup);
//float annemarieEnergy( const edm::PtrVector<PFRecHit>& rechitvec, const edm::Ptr<reco::SuperCluster>& sc, 	std::vector<std::string> geometrySource_, const edm::Event& iEvent, const edm::EventSetup& iSetup, float eta_ECAL, int sig=1);
//		float phiCrackDistance (float x, float y);
//		void getMaximumCell(const edm::PtrVector<PFRecHit>& rechitvec,const double & phimax,const    double & etamax,std::vector<HGCEEDetId> & detidmax);
...
double sigmaetaeta(const reco::PFCluster& clu,const  edm::PtrVector<reco::PFRecHit>& rcs );
	...
	//	edm::EDGetTokenT<edm::View<reco::PFRecHit> >endcapRecHitCollection_      ; 
  ...
  std::string g4TracksSource_, g4VerticesSource_; 
	...
	//	edm::Handle<edm::PtrVector<reco::PFRecHit> > recHits_;	
		int PU;
		int process;
		int genMatchPU;
  double cellSize_;
...
// constructor
HoverEAnalyzer_Phase1::HoverEAnalyzer_Phase1(const edm::ParameterSet& iConfig):
	...
	endcapClusterCollection_(consumes <edm::View<reco::PFCluster> > (iConfig.getUntrackedParameter<edm::InputTag>("endcapClusterCollection",edm::InputTag("particleFlowClusterHCAL")))),
	...
	eeRecHitCollection_(consumes <edm::View<reco::PFRecHit> > (iConfig.getUntrackedParameter<edm::InputTag>("eeRecHitCollection",edm::InputTag("particleFlowRecHitECAL:Cleaned"))))

{
  MVAweightfile_ = iConfig.getParameter<edm::FileInPath>( "MVAweightfile" );
	...
	Mva_->AddVariable( "sigmaIetaIeta", &sigmaIetaIeta_ );
  ...
  g4VerticesSource_         = iConfig.getUntrackedParameter<std::string>("g4VerticesSource");
	...
	treePhoton->Branch("sigmaIetaIeta"        ,&infoPhoton.sigmaIetaIeta        ,"sigmaIetaIeta/F");
	...
	treeBackground->Branch("sigmaIetaIeta"        ,&infoBackground.sigmaIetaIeta        ,"sigmaIetaIeta/F");
...
// destructor
HoverEAnalyzer_Phase1::~HoverEAnalyzer_Phase1()
{

}


/*double HoverEAnalyzer_Phase1::fillEmIdVars(const edm::Ptr<reco::SuperCluster>& sc,const edm::PtrVector<reco::PFCluster>& clusters,const edm::PtrVector<PFRecHit> &  rcs, 	const HGCalGeometry *geom,const edm::Event& iEvent, const edm::EventSetup& iSetup ){
	...
	for (unsigned int j =0 ; j < clusters.size() ; j++){
		...
		if (clusters[j]->position()==(sc->seed()->position())) {
		  ...
		  pcaShowerAnalysis.showerParameters((clusters[j].get()),pcaShowerPos,pcaShowerDir);
			...
			infoBackground.sigmaIetaIeta = test.HGCALShowerBasedEmIdentificationLC2::sigmaetaeta( *(clusters[j].get()), rcs);
			infoSuperCluster.sigmaIetaIeta = infoBackground.sigmaIetaIeta;  
			...
			infoSuperCluster.lengthCompatibility = infoBackground.lengthCompatibility;

			break;
		}
	}

	return infoPhoton.sigmaIetaIeta;
 }*/


	void
HoverEAnalyzer_Phase1::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{	
...
//	Handle<edm::SortedCollection<PFRecHit,edm::StrictWeakOrdering<PFRecHit> >  > HGCEERechits_;
//	iEvent.getByToken(endcapRecHitCollection_,HGCEERechits_);
//	auto  HGCEERechits =HGCEERechits_.product();
	//const edm::SortedCollection<PFRecHit>& rechits = HGCEERechits->product();

	Handle<edm::View<reco::Photon> > HGCEESCs;
	iEvent.getByToken(endcapPhotonCollection_,HGCEESCs);
	const PtrVector<reco::Photon>& sclusters = HGCEESCs->ptrVector();
  ...
  const edm::PtrVector<reco::PFRecHit>& rechitvec = eeRecHits->ptrVector();
	if (rechitvec.size());
  ...
  iEvent.getByLabel(g4VerticesSource_,SimVtx);
	...
	std::cout << "debug 02=======================================================" << std::endl;
		for (unsigned int isc =0; isc < sclusters.size() ; isc++){ //subloop over sc's to find matches
			...
			int detector = sclusters[isc]->superCluster()->seed()->hitsAndFractions()[0].first.subdetId() ;
			if (detector);

			//if( detector!=HGCEE) continue; //FIXME

			float dE = (sclusters[isc]->superCluster()->seed()->eta() - eta_ECAL);
			dE =dE*dE;
			float dP = deltaPhi(sclusters[isc]->superCluster()->seed()->phi() , gens[igp]->phi());
			dP =dP*dP;
			float dR = sqrt(dE +dP);
...
int detectorSC = sclusters[isc]->superCluster()->seed()->hitsAndFractions()[0].first.subdetId() ;
...
/*float HoverEAnalyzer_Phase1::annemarieEnergy( const edm::PtrVector<PFRecHit>& rechitvec, const edm::Ptr<reco::SuperCluster>& sc, 	std::vector<std::string> geometrySource_, const edm::Event& iEvent, const edm::EventSetup& iSetup, float eta_ECAL, int sig){
	...
	GlobalPoint pcaShowerPos;
  GlobalVector pcaShowerDir;
	pcaShowerAnalysis.showerParameters((sc->superCluster()->seed().get()),pcaShowerPos,pcaShowerDir);


	double clus_eta = sc->superCluster()->seed()->eta();
	...
	for (int iLayer =0; iLayer < 30 ; iLayer++){
		...
		if (topology.valid(detidmax[iLayer])) {//fillNeighbours(detidmax[iL],Exy,info);
			...
			for (unsigned idx(0);idx<9;++idx){
				...
				PFRecHitCollection::const_iterator theHit = recHits_->find(neighbours[idx]);
...
float HoverEAnalyzer_Phase1::phiCrackDistance (float x, float y){
	float d=999;
	float dmin=999;
	for (int N =0; N< 9; N++){
...
void HoverEAnalyzer_Phase1::getMaximumCell(const edm::PtrVector<PFRecHit>& rechitvec,const double & phimax,const double & etamax,std::vector<HGCEEDetId> & detidmax){
	...
	for (unsigned iH(0); iH<rechitvec.size(); ++iH){//loop on rechits
		const PFRecHit & lHit = *(rechitvec[iH]);
...
double HoverEAnalyzer_Phase1::absWeight(const unsigned layer, const bool dedx){
	if (dedx==false){
...
void HoverEAnalyzer_Phase1::fillDetIdStack(std::vector<HGCEEDetId> & detidmax_vec, std::vector<HGCEEDetId> & detidstack){
	unsigned int nLayers_=30;
...
double HoverEAnalyzer_Phase1::sigmaetaeta(const reco::PFCluster& clu,const  edm::PtrVector<reco::PFRecHit>& rcs )
{
...
//				refhit  = rcs[irh];
  ...
  double mip_ = 0.0000551;
  double minenergy_ = 4.;
  //double rmax_ = 100.; // no transverse limitation for no PU case
...
// ------------ method called once each job just before starting event loop  ------------
	void 
HoverEAnalyzer_Phase1::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
	void 
HoverEAnalyzer_Phase1::endJob() 
{
}

// ------------ method called when starting to processes a run  ------------
	void 
HoverEAnalyzer_Phase1::beginRun(edm::Run const&, edm::EventSetup const& iSetup)
{
	edm::ESHandle<HGCalGeometry> hgcGeo;
	iSetup.get<IdealGeometryRecord>().get(geometrySource_[0],hgcGeo);
...
// ------------ method called when ending the processing of a run  ------------
	void 
HoverEAnalyzer_Phase1::endRun(edm::Run const&, edm::EventSetup const&)
{
}

// ------------ method called when starting to processes a luminosity block  ------------
	void 
HoverEAnalyzer_Phase1::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}

// ------------ method called when ending the processing of a luminosity block  ------------
	void 
HoverEAnalyzer_Phase1::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
HoverEAnalyzer_Phase1::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(HoverEAnalyzer_Phase1);
