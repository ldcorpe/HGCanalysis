...
struct info_t {
  ...
  void correctForEtaDep(const ROOT::Math::XYZVector & posSC){
    ...
    for (unsigned i(0);i<5;++i){
      eReco[i]  = eReco[i]/fabs(tanh(etaReco));         
      eRecoCleaned[i]  = eRecoCleaned[i]/fabs(tanh(etaReco));         
    }
  ...
  void fillTruth(const edm::Ptr<reco::GenParticle> & aPhoton){
